    ...
    if not bx.available():
        ...
        return {"extracted": 0, "pending": len(files),
                "error": f"AI 还不能用({st['label']},当前 {st['provider']})—— "
                         f"原文已留在队列里,一条都没丢。点右上角「AI …」那颗胶囊,"
                         f"选一家、填 key、测一下,再回来点提取。"}
...
# ── AI 模型设置(自带,不依赖别的服务)────────────────────────
#
# 密钥写在本仓库的 .env,这里给页面入口。三条铁律(沿用上游验证过的写法):
#   1. **绝不回传明文** —— 只回「配没配」和尾四位
#   2. 只动白名单键,不让页面变成任意写文件的口子
#   3. 写回时保留 .env 里其它行和注释,不整个重写;写完 chmod 0600

_EDITABLE = {
    "LLM_PROVIDER": "用哪家模型:qwen / minimax / deepseek / moonshot / zhipu / ollama",
    "LLM_API_KEY": "上面那家的 API Key(保存时会自动落到该家的专用键上)",
    "LLM_MODEL": "覆盖默认模型名(留空用默认;各家改版勤,不通就来这里改)",
    "LLM_MODEL_FAST": "快档模型:提取/匹配用(qwen 默认 qwen-flash,实测比 plus 快 2.6 倍)",
    "DASHSCOPE_API_KEY": "通义千问 API Key",
    "MINIMAX_API_KEY": "MiniMax API Key",
    "DEEPSEEK_API_KEY": "DeepSeek API Key",
    "MOONSHOT_API_KEY": "月之暗面 API Key",
    "ZHIPU_API_KEY": "智谱 API Key",
    "EMBED_MODEL": "嵌入模型(默认 BAAI/bge-m3;换了要重建索引,会自动提示)",
}
_SECRET_KEYS = {"LLM_API_KEY"} | {k for k in _EDITABLE if k.endswith("_API_KEY")}
...
@app.get("/api/boss/llm/providers")
async def llm_providers() -> dict[str, Any]:
    """能选哪些供应商 —— 都是 OpenAI 兼容端点,换家只改配置。"""
    import llm as _llm
    cur = _llm.config()
    return {
        "providers": [
        ...
        "current": {"provider": cur["provider"], "model": cur["model"],
                    "fast_model": _llm.fast_model(),
                    "key_set": bool(cur["_key"]), "key_hint": _mask(cur["_key"])},
        "status": _llm.status(),
    }


@app.post("/api/boss/llm/test")
...
@app.post("/api/boss/settings")
async def post_settings(body: dict[str, Any] = Body(...)) -> dict[str, Any]:
    """写回 .env。只认白名单键,保留文件里其它行和注释。"""
    body = dict(body or {})

    # 页面上的「API Key」跟着当前选的供应商走,写到那家自己的键上,
    # 而不是全局 LLM_API_KEY —— 否则切一次供应商就把 key 带过去了
    # (上游实测:切到智谱还在发 MiniMax 的 key,对方回「令牌已过期」,极其误导)。
    ...
    path.write_text("\n".join(out) + "\n", encoding="utf-8")
    try:
        path.chmod(0o600)      # 里面有密钥,别让同机其它用户读到
    except OSError:
        pass
    config.reload()
    import llm as _llm
    return {"saved": sorted(updates), "status": _llm.status()}
